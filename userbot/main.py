# Copyright (C) 2021 Microser UserBot
#
# Licensed under the GPL-3.0 License;
# you may not use this file except in compliance with the License.
#
# MicroserUserBot - TheRaj
...
from . import LOGS, bot, PLUGIN_CHANNEL_ID, CMD_HELP, LANGUAGE, MİCROSER_VERSION, PATTERNS
...
DIZCILIK_STR = [
    ...
    "Aga çalıom ya kb neyse dız time.",
]
...
def extractCommands(file):
    ...
    if re.search(r'CmdHelp\(.*\)', FileRead):
        ...
        for Command in Pattern:
            ...
            # MicroserPY
            Microserpy = re.search('\"\"\"MİCROSERPY(.*)\"\"\"', FileRead, re.DOTALL)
            if not Microserpy == None:
                Microserpy = Microserpy.group(0)
                for Satir in Microserpy.splitlines():
...
try:
    bot.start()
    idim = bot.get_me().id
    #Microserbl = requests.get('https://raw.githubusercontent.com/MicroserBOTCompany/dosyalar/main/blacklist.json').json()
    #if idim in Microserbl:
        #bot.disconnect()
    ...
    PLUGIN_MESAJLAR = {}
    ORJ_PLUGIN_MESAJLAR = {"alive": "`💋 MicroserUserBot Calışıyor...😎`", "afk": f"`{str(choice(AFKSTR))}`", "kickme": "`Güle Güle ben gidiyorum `🤠", "pm": UNAPPROVED_MSG, "dızcı": str(choice(DIZCILIK_STR)), "ban": "{mention}`, yasaklandı!`", "mute": "{mention}`, sessize alındı!`", "approve": "{mention}`, bana mesaj gönderebilirsin!`", "disapprove": "{mention}`, artık bana mesaj gönderemezsin!`", "block": "**#ENGELLENDI**\n`Kullancı: `{mention}\n`Sebebi: {sebep}`", "unblock": "[{name0}](tg://user?id={replied_userid}) kişisinin engeli kaldırıldı."}

    PLUGIN_MESAJLAR_TURLER = ["alive", "afk", "kickme", "pm", "dızcı", "ban", "mute", "approve", "disapprove", "block", "unblock"]
...
LOGS.info("Botunuz çalışıyor! Herhangi bir sohbete .alive yazarak Test edin."
          " Yardıma ihtiyacınız varsa, Destek grubumuza gelin t.me/MicroserSupport")
LOGS.info(f"Bot sürümünüz: Microser {MİCROSER_VERSION}")
